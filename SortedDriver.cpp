...
double
mostIsolated(vector<double> & number, double& element, double& dist)
{
	// STUB  STUB  STUB
	...
	if (size > 2)
	{
		for (int i = 1; i < size-1; i++)
		{
			...
			if (dist < sqrt(pow(number[i-1],2) + pow(number[i + 1],2))) //This uses the distance formula to determine which element is the greatest distance from other elements
			{
				
				dist = sqrt(pow(number[i - 1], 2) + pow(number[i + 1], 2));
				element = i;
				cout << dist;
			}
				cout << sqrt(pow(number[i - 1], 2) + pow(number[i + 1], 2)) << endl;
		}
		return element;
	}
...
main()
{
	...
	while (true)
	{
		...
		Timer time;
		time.start();
		double isolated = mostIsolated(numbers, element, dist);
		time.stop();
